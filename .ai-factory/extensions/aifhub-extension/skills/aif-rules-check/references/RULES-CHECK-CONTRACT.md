...
Schema reminder: `"status": "pass|warn|fail"`, `"blocking": true|false`, `"blockers": []`, `"affected_files": []`, `"suggested_next": {}` or `null`.
...
The human rules report keeps `PASS` / `WARN` / `FAIL`. The final machine-readable summary uses lowercase `pass` / `warn` / `fail` in the `aif-gate-result` JSON block.

## Read-Only Contract

- `/aif-rules-check` does not edit rule artifacts, generated rules, source files, runtime state, or QA evidence.
