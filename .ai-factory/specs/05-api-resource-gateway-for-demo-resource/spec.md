---
artifact_type: spec
plan_id: "05-api-resource-gateway-for-demo-resource"
title: "API resource gateway for Example App"
artifact_status: archived
owner: aif-verify
created_at: 2026-04-25
updated_at: 2026-04-26
source_issue: null
source_plan: "05-api-resource-gateway-for-demo-resource"
---

# Spec: 05-api-resource-gateway-for-demo-resource

> Finalized specification archived from plan verification.

## Summary

Implemented an OpenResty gateway reference for `api.example.test`: local RS256 JWT validation against Laravel JWKS, fail-closed token and claim checks, sanitization of client-supplied `X-Idshka-*`, trusted context header injection, minimal internal `demo-resource-api` upstream, reproducible gateway smoke script, CI smoke integration, and documentation sync.
...
- [x] Added internal `demo-resource-api` echo upstream for smoke verification.
...
|------|-------------|-------------|
| `infra/openresty/demo-resource/Dockerfile` | created | Pins the OpenResty image path and installs `openssl` explicitly. |
| `infra/openresty/demo-resource/nginx.conf` | modified | Defines `/healthz`, internal JWKS subrequest, JWT validation, trusted header injection, and proxying to `demo-resource-api`. |
| `infra/openresty/demo-resource/lua/jwks_cache.lua` | created | Fetches and caches public JWKS keys by `kid`. |
| `infra/openresty/demo-resource/lua/jwt_validate.lua` | created | Validates JWT structure, signature, issuer, audience, required claims, and validity windows. |
| `infra/openresty/demo-resource/lua/context_headers.lua` | created | Sanitizes spoofed auth context and writes gateway-generated `X-Idshka-*` headers. |
| `examples/demo-resource-api/nginx.conf` | created | Provides an internal upstream endpoint for smoke tests. |
| `infra/openresty/demo-resource/smoke.sh` | created | Runs the reproducible gateway contract smoke flow. |
| `routes/console.php` | modified | Adds local-only smoke token issuance command. |
| `compose.yml` | modified | Builds the gateway image and adds internal `demo-resource-api`. |
...
| `bash infra/openresty/demo-resource/smoke.sh "docker compose -p idshka_aif_verify"` | passed |
...
3. **Keep `demo-resource-api` internal-only** so the example demonstrates the trusted gateway boundary.
...
| Type | Reference |
|------|-----------|
| Original Plan | `.ai-factory/plans/05-api-resource-gateway-for-demo-resource/` |
| Plan File | `.ai-factory/plans/05-api-resource-gateway-for-demo-resource.md` |
| Issue | `n/a` |
| PR | `n/a` |
| Branch | `feature/05-api-resource-gateway-for-demo-resource` |
