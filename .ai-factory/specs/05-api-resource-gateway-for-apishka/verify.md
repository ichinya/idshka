...
- [x] Gateway config has explicit `issuer`, `audience=apishka.ru`, internal `jwks_url`, allowed `alg`, JWKS TTL and stale/fail-closed policy.
...
- [x] Valid user API token for `aud=apishka.ru` reaches upstream through gateway.
...
- [x] Minimal `apishka-api` smoke upstream exists and can prove header sanitization/replacement.
...
- [x] `docs/API_FLOWS.md`, `docs/GATEWAY_CONTRACT.md`, `docs/README.md`, and `infra/openresty/apishka/lua/README.md` match implemented gateway behavior.
...
- 2026-04-26 fresh verification: `bash infra/openresty/apishka/smoke.sh "docker compose -p idshka_aif_verify"` passed in a fresh Compose project.
