...
`apishka.ru` — внешний сайт пользователя. Его gateway-конфиг лежит как reference example.
...
- `examples/apishka-api` — минимальный API, который читает `X-Idshka-*` и/или проверяет `X-Idshka-Context-Signature`.
- `examples/apishka-web-laravel` — Laravel web-сайт, который входит через `idshka.ru` с кастомным Socialite provider или обычным OAuth callback controller.
