...
## Важная граница
Socialite не выпускает токены для `example.test` и не публикует JWKS. Это делает `Issuer` domain внутри Laravel.

## `example.test` как Laravel web-client
...
1. `example.test` генерирует `state`, `nonce`, PKCE verifier и S256 challenge.
...
4. `example.test` вызывает `POST https://idshka.ru/oauth/token` с client secret и PKCE verifier.
5. `example.test` валидирует `id_token` через `GET /oauth/jwks.json`, вызывает `GET /oauth/userinfo` с web access token и открывает локальную session.
...
- Provider contract для `example.test` как web-client реализован через Authorization Code + PKCE.
