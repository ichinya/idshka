...
final class EloquentVerifiedSiteLookup implements VerifiedSiteLookup
{
    ...
    {
        Log::debug('[site.lookup.verified] started', SafeLogContext::from(['site_id' => $siteId]));

        $site = Site::query()->findOrFail($siteId);

        if (! $site->isVerified()) {
            Log::warning('[site.lookup.verified] rejected_unverified_site', SafeLogContext::from([
                'site_id' => $siteId,
        ...
        Log::debug('[site.lookup.verified] completed', SafeLogContext::from(['site_id' => $siteId]));
