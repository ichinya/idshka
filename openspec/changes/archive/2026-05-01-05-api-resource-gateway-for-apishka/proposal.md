# Proposal: 05-api-resource-gateway-for-apishka

## Intent

- [x] **T2. OpenResty routing skeleton и upstream boundary**
  - Заменить текущий `501 gateway_not_implemented` в `infra/openresty/apishka/nginx.conf` на proxy flow через `access_by_lua*`.
  - Сохранить `/healthz` без auth и с security headers.
  - Настроить upstream для минимального `apishka-api` consumer/echo service.
...
## Цель
Собрать OpenResty gateway example для `api.apishka.ru`, который локально проверяет user API JWT от `idshka.ru`, fail-closed отклоняет недоверенные запросы, затирает входящие `X-Idshka-*` и прокидывает в upstream только подтвержденный gateway context.

## Area
`gateway`, `api_resource`, `security`, `ops`

## Зависимости
- `04-token-issuer-and-jwks` должен быть завершен: доступен выпуск user API token для `aud=apishka.ru` и public `GET /oauth/jwks.json`.
...
- Не в scope:
  ...
  - Полноценный product-grade API `apishka.ru`.
...
- [x] **T7. Minimal `apishka-api` smoke upstream**
  - Добавить минимальный upstream consumer/echo endpoint в `examples/apishka-api` или отдельный lightweight service в Compose.
...
- [x] **T8. Gateway smoke scripts и CI integration**
  - Добавить воспроизводимый способ подготовить local valid token:
    ...
    - создать signing key;
    - выпустить user API token для `aud=apishka.ru`.
...
- [x] **T9. Docs sync и implementation notes**
  ...
  - Обновить `infra/openresty/apishka/lua/README.md` с назначением модулей и security assumptions.
...
## Acceptance criteria
- [x] Valid user API token для `aud=apishka.ru` проходит через gateway до upstream.
