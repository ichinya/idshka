...
export async function getOpenSpecStatus(changeId, options = {}) {
  ...
  if (result.ok || !shouldUseFilesystemStatusFallback(result, changeId)) {
    return result;
  }

  return createFilesystemStatusFallback(result, changeId, options);
}
...
function shouldUseFilesystemStatusFallback(result, changeId) {
  const output = `${result?.stdout ?? ''}\n${result?.stderr ?? ''}`;

  return isSafeChangeId(changeId)
    && /Invalid change name/i.test(output)
    && /must start with a letter/i.test(output);
}

function createFilesystemStatusFallback(result, changeId, options = {}) {
  const cwd = path.resolve(options.cwd ?? process.cwd());
  const changesDir = path.resolve(cwd, 'openspec', 'changes');
  const changeDir = path.resolve(changesDir, changeId);

  if (!isWithinDirectory(changeDir, changesDir) || !isDirectorySync(changeDir)) {
    return result;
  }

  const tasks = parseTaskStatus(readOptionalFileSync(path.join(changeDir, 'tasks.md')));
  const json = {
    change: changeId,
    source: 'filesystem-fallback',
    reason: 'openspec-status-rejects-numeric-change-id',
    tasks
  };
...
function parseTaskStatus(markdown) {
  const items = [];
...
function readOptionalFileSync(filePath) {
  try {
